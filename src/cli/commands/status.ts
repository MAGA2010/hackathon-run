...
function summarize(file: FileName, raw: unknown): string[] {
  ...
  if (file === 'plan.json') {
    const features = Array.isArray(d.features)
      ? (d.features as Array<Record<string, unknown>>)
      : [];
  ...
  if (file === 'verify.json') {
    ...
    return [
      'status: ' + status,
      'steps: ' + pass + ' pass / ' + fail + ' fail (of ' + steps.length + ')',
    ];
  }
  if (file === 'demo.json') {
  ...
  if (file === 'review.json') {
    const dims = Array.isArray(d.dimensions)
      ? (d.dimensions as Array<Record<string, unknown>>)
      : [];
    const overall = typeof d.overall === 'number' ? d.overall : 0;
    const fp =
      d.fix_priorities && typeof d.fix_priorities === 'object'
        ? (d.fix_priorities as Record<string, unknown>)
        : null;
    const fixNow = fp && Array.isArray(fp.fix_now) ? (fp.fix_now as unknown[]).length : 0;
    return [
      'overall: ' + overall + ' / 5 across ' + dims.length + ' dimensions',
      'fix_now items: ' + fixNow,
    ];
  }
  if (file === 'ship.json') {
    const ss =
      d.secret_scan && typeof d.secret_scan === 'object'
        ? (d.secret_scan as Record<string, unknown>)
        : null;
    const clean = ss ? ss.clean : null;
    const findings = ss && Array.isArray(ss.findings) ? (ss.findings as unknown[]).length : 0;
    const cl =
      d.checklist && typeof d.checklist === 'object'
        ? (d.checklist as Record<string, unknown>)
        : null;
    ...
    out.push(
      'secret_scan: ' +
        (clean === true ? 'clean' : clean === false ? 'DIRTY (' + findings + ' findings)' : 'n/a'),
    );
