...
describe('scope-knife WSJF (v1.2.1.6)', () => {
  ...
  it('matches demo-goal verbs to feature names with light stemming', () => {
    assert.match(script, /def stem_word\(word: str\) -> str:/);
    assert.match(script, /def demo_relevance\(demo_goal: str, feature_name: str\) -> int:/);
    assert.match(script, /goal_stems\s*& name_stems/);
  });
