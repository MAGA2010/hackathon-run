...
export function doctor(opts: { cwd: string; json?: boolean }): number {
  ...
  const schemas: string[] = existsSync(schemaDir)
    ? readdirSync(schemaDir).filter((n) => n.endsWith('.schema.json'))
    : [];
  if (schemas.length === 0) {
    checks.push({
      name: 'schemas',
    ...
    if (badSchema === 0) {
      checks.push({
        name: 'schemas',
        severity: 'ok',
        message: schemas.length + ' schema files valid JSON',
      });
    }
  }
  if (existsSync(stateDir) && schemas.length > 0) {
    ...
    if (badState === 0) {
      checks.push({
        name: 'state-validate',
        severity: 'ok',
        message: 'every state file matches its schema',
      });
    }
  }
  ...
  if (opts.json) {
    ...
    if (failCount > 0) {
      ...
      console.log(
        c.red(
          '[ERR] ' + failCount + ' check(s) failed. The CLI may misbehave until they are fixed.',
        ),
      );
    } else if (warnCount > 0) {
      console.log();
      console.log(
        c.yellow(
          '[!] ' +
            warnCount +
            ' warning(s). Skills will degrade gracefully but some features may be unavailable.',
        ),
      );
    } else {
      console.log();
