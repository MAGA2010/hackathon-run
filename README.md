...
`hackathon mcp` exposes the same 25 tools over JSON-RPC 2.0, but MCP calls no
longer scrape CLI stdout. CLI commands return typed result payloads, and the
MCP layer validates every tool argument against its JSON Schema before
dispatch.

Tool results include both backward-compatible text JSON and MCP
`structuredContent`. Expected command failures set `isError: true` instead
of becoming transport errors. `apply_skill_advice` validates the target state
schema and writes through the same cross-process lock and atomic rename path
as the CLI.

See [MCP protocol](docs/architecture/mcp-protocol.md) for the response shape
